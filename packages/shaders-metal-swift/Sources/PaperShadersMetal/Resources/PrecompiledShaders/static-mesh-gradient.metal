...
fragment main0_out main0(main0_in in [[stage_in]], constant Params& params [[buffer(0)]])
{
    ...
    {
        uv.x += (((params.u_waveX * center) / i) * cos((6.2831855 * params.u_waveXShift) + ((i * 2.0) * smoothstep(0.0, 1.0, uv.y))));
        uv.y += (((params.u_waveY * center) / i) * cos((6.2831855 * params.u_waveYShift) + ((i * 2.0) * smoothstep(0.0, 1.0, uv.x))));
    }
    float3 color = float3(0.0);
    float opacity = 0.0;
    float totalWeight = 0.0;
    float positionSeed = 25.0 + (0.33 * params.u_positions);
    for (int i_1 = 0; i_1 < 10; i_1++)
    {
        ...
        int param_2 = i_1;
        float param_3 = positionSeed;
        float2 pos = getPosition(param_2, param_3) + float2(mixerGrain);
        float dist = length(uv - pos);
        dist = length(uv - pos);
        float3 colorFraction = params.u_colors[i_1].xyz * params.u_colors[i_1].w;
        float opacityFraction = params.u_colors[i_1].w;
        float mixing = powr(params.u_mixing, 0.7);
        float power = mix(2.0, 1.0, mixing);
        dist = powr(dist, power);
        float w = 1.0 / (dist + 0.001);
        float baseSharpness = mix(0.0, 8.0, fast::clamp(w, 0.0, 1.0));
    ...
    float param_5 = 1.0;
    float2 param_6 = rotate(param_4, param_5) + float2(3.0);
    float grainOverlay = valueNoise(param_6);
    float2 param_7 = grainUV;
    float param_8 = 2.0;
    float2 param_9 = rotate(param_7, param_8) + float2(-1.0);
    grainOverlay = mix(grainOverlay, valueNoise(param_9), 0.5);
